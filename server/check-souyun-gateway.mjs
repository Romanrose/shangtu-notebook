...
// 1. 槽位抽取。
textEqual(JSON.stringify(extractSlots("李白写过《将进酒》吗")), JSON.stringify({ person: "李白", work: "将进酒" }), "书名号问句抽取失败");
textEqual(JSON.stringify(extractSlots("李白写过将进酒吗？")), JSON.stringify({ person: "李白", work: "将进酒" }), "无书名号问句抽取失败");
textEqual(JSON.stringify(extractSlots("李白是否写过《将进酒》")), JSON.stringify({ person: "李白", work: "将进酒" }), "是否式问句抽取失败");
textEqual(JSON.stringify(extractSlots("李白有没有写过将进酒")), JSON.stringify({ person: "李白", work: "将进酒" }), "有没有式问句抽取失败");
textEqual(JSON.stringify(extractSlots("苏轼写过《前赤壁赋》吗")), JSON.stringify({ person: "苏轼", work: "前赤壁赋" }), "苏轼问句抽取失败");
...
console.log("Souyun CNKGraph gateway contract verified: 作者漂移过滤（同姓陷阱与换行名字）、双响应形态、证据转换、来源钳制、缓存边界与 HTTP 鉴权均通过。");
