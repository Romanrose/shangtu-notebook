...
| 有歧义页边回应 | 使用仅服务端的 `SABER_PI_FIXTURE_SCENARIO=ambiguous` 固定演练场景，默认确认文本后显示问题和“李白、李贺”候选。 |
| 有缺口页边回应 | 使用仅服务端的 `SABER_PI_FIXTURE_SCENARIO=gap` 固定演练场景，默认确认文本后明确显示“当前演练图谱没有这条可核验的直接关联”。 |
...
`有歧义` 与 `有缺口` 还由 Node fixture 和 Flutter Dart live smoke 覆盖。平板 UI 录制使用的固定 scenario 只解决 Android shell 不能注入中文确认文本的自动化限制；它不能作为真实实体澄清或真实证据缺口质量的证明。
